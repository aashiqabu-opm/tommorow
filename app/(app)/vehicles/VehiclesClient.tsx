...
import { Plus, Car, Pencil, Trash2, ChevronDown, ChevronRight, Fuel, Route, Wrench } from 'lucide-react'
...
import type { Vehicle, VehicleLog, VehicleType, VehicleLogType } from '@/lib/types'
...
const V_EMPTY = { reg_number: '', name: '', vehicle_type: 'car' as VehicleType, ownership: 'owned', owner_name: '', hire_rate: '', hire_basis: 'day', driver_name: '', driver_phone: '', project_id: '', status: 'active', notes: '' }
...
export function VehiclesClient({ vehicles, projects, userId, canDelete }: Props) {
  ...
  function openEdit(v: Vehicle) {
    ...
    setForm({
      ...
      ownership: v.ownership ?? 'owned', owner_name: v.owner_name ?? '',
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard title="Hired" value={vehicles.filter(v => v.ownership === 'hired').length} status="default" subtitle="of fleet" />
      </div>
          ...
          {shown.map(v => {
            ...
            return (
              <div key={v.id} className="bg-[#13131a] border border-[#2a2a3a] rounded-2xl overflow-hidden">
                <div className="px-5 py-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-[#8888aa] flex flex-wrap gap-x-3 gap-y-0.5">
                        ...
                        {v.ownership === 'hired' && v.owner_name && <span>Hired from {v.owner_name}{v.hire_rate ? ` · ${formatCurrency(v.hire_rate)}/${v.hire_basis}` : ''}</span>}
                      </div>
                    </div>
                  ...
                  <div className="mt-3 flex items-center gap-4">
                    <button onClick={() => { setExpanded(isOpen ? null : v.id) }} className="text-xs text-[#8888aa] hover:text-white inline-flex items-center gap-1">
                      {isOpen ? <ChevronDown size={13} /> : <ChevronRight size={13} />} Logs ({(v.logs ?? []).length})
                    </button>
                ...
                {isOpen && (
                  <div className="border-t border-[#2a2a3a] bg-[#0f0f16] px-5 py-3">
                    {logs.length === 0 ? <p className="text-xs text-[#5a5a7a] py-2">No logs yet.</p> : (
